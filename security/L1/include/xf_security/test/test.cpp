...
struct sha256_wrapper {
    // lW=64 bits, BLOCK_SIZE=64 bytes, hshW = 256 bits
...
void test_hmac_sha256(hls::stream<ap_uint<KEYW> >& keyStrm,
                      hls::stream<ap_uint<MSGW> >& msgStrm,
                      hls::stream<u64>& lenStrm,
                      hls::stream<bool>& eLenStrm,
                      hls::stream<ap_uint<HSHW> >& hshStrm,
                      hls::stream<bool>& eHshStrm) {
    xf::security::hmac<MSGW, LENW, HSHW, KEYL, BLOCK_SIZE, sha256_wrapper>(keyStrm, msgStrm, lenStrm, eLenStrm, hshStrm,
                                                                           eHshStrm);
}
...
void hmacSHA256(const unsigned char* key,
    ...
    if (keyLen > BLOCK_SIZE) {
        SHA256((const unsigned char*)key, keyLen, (unsigned char*)h1);
    ...
    SHA256((const unsigned char*)kmsg, BLOCK_SIZE + msgLen, (unsigned char*)h2);
...
int main() {
    std::cout << "********************************" << std::endl;
    std::cout << "   Testing hmac+SHA256 on HLS project   " << std::endl;
    std::cout << "********************************" << std::endl;
...
#pragma HLS stream variable = endHshStrm depth = 128
    ...
    // call fpga module
    test_hmac_sha256(keyStrm, msgStrm, lenMsgStrm, endLenStrm, hshStrm, endHshStrm);
